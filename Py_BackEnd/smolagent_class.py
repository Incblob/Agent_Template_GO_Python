    """A Class for creating a smiple smolagent multi-agent group. This can be used
...
class SmolAgentMulti():
    ...
    To use, initialize the class, and inject the db connection
    ...
    def add_manager(self, agent:CodeAgent, managed_agents:List[str]= []):
        assert self.sub_agents != False, ValueError("manager agent expects sub_agents, please add those first")
        assert self.manager == None, ValueError("manager already set") # we are assuming one manager for simplicity
    ...
    @staticmethod
    def agent_factory(tools:list[Tool], name:str, description: str , verbosity :int= 0, max_steps:int=2, **kwargs: Any):
        return CodeAgent(
